...
bool isValidMove(int piece_code, int from_x, int from_y, int to_x, int to_y)
{
    ...
    case TYPE_XIANG: // 象/相
                     ...
                     //     }
        // 不能过河
        if (color == COLOR_RED && to_x < 5)
            return false;
        if (color == COLOR_BLACK && to_x > 4)
            return false;
        // 田字走法
    ...
    case TYPE_MA: // 马
        ...
        {
            int block_x = from_x + dx / 2;
            if (board[block_x][from_y] != NONE)
                return false;
        }
        else
        {
            int block_y = from_y + dy / 2;
            if (board[from_x][block_y] != NONE)
                return false;
        }
        return true;
    ...
    case TYPE_JU: // 车
        ...
        if (from_x != to_x && from_y != to_y)
            return false;
        // 路径上不能有其他棋子
        if (countPiecesInLine(from_x, from_y, to_x, to_y) > 0)
            return false;
        return true;
    ...
    case TYPE_BING: // 兵/卒
        ...
        if (color == COLOR_RED)
        {
            if (dx > 0)
                return false; // 红兵不能后退
            if (from_x > 4 && abs_dx > 1)
                return false; // 过河前只能走一步
        }
        else
        {
            if (dx < 0)
                return false; // 黑卒不能后退
            if (from_x < 5 && abs_dx > 1)
                return false; // 过河前只能走一步
        }
        // 只能走一步
        if (abs_dx + abs_dy != 1)
        ...
        if (abs_dy > 0)
        {
            if (color == COLOR_RED && from_x >= 5)
                return false; // 红兵没过河
            if (color == COLOR_BLACK && from_x <= 4)
                return false; // 黑卒没过河
        }
        return true;
...
void revokeLastMove()
{
    ...
    // 如果被吃掉的是将或帅，恢复其存活状态
    if (last_move->captured_piece == 11)
    { // RED_SHUAI = 11
        is_shuai_live = true;
        blackFlyToWin = false; // 重置黑方飞将胜利标志
        // printf("恢复红帅存活状态\n");
    }
    else if (last_move->captured_piece == 21)
    { // BLACK_JIANG = 21
        is_jiang_live = true;
